...
Chaque ETP enseignant (etp_enseignants, 1er + 2nd degré) devient un nœud Inconnu
...
Modèle :
    (:Inconnu {id_inconnu, corps, genre, tranche_age, tranche_anciennete, ...})
...
def _inconnu_records(row, uai, degre, n, ens):
    """n nœuds Inconnu pour un établissement + leurs relations TRAVAILLE_DANS."""
    seqs = {
    ...
    for k in range(n):
        ...
        edges.append({"sourceId": iid, "targetId": uai,
                      "source": "effectifs_EN_2024", "quotite": 1.0, "annee": ANNEE})
    return inconnus, edges


def build_records(limit=None):
    """Lit le CSV et construit les records établissements + Inconnus ventilés.

    Ne garde que les degrés 1d/2d ayant round(etp_enseignants) >= 1.
    Chaque ETP arrondi génère un nœud Inconnu avec ventilation démographique.
    Les Inconnus couverts par des contacts SNUPers seront supprimés par reconcile_snupers.py.
    """
    df = pd.read_csv(CSV_PATH, low_memory=False)
    df = df[df["degre"].isin(DEGRES)]

    etabs, inconnus, edges = [], [], []
    for row in df.to_dict("records"):
        ...
        row_inconnus, row_edges = _inconnu_records(row, uai, degre, effectif, ens)
        inconnus.extend(row_inconnus)
        edges.extend(row_edges)

        if limit is not None and len(etabs) >= limit:
            break

    return etabs, inconnus, edges
...
def main():
    ...
    etabs, inconnus, edges = build_records(limit)
    geo = build_geo_records(etabs)
    print(f"  -> {len(etabs)} établissements, {len(inconnus)} inconnus")
    ...
    with GraphDatabase.driver(uri, auth=auth) as driver:
        ...
        _run_batches(driver, db, INCONNU_CYPHER, inconnus, "Inconnu")
